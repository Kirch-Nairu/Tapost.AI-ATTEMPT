...
export const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
  const { startTaskSession, markTaskCompleted, deleteTask, setCurrentScreen, activeTaskId, activeRemainingSeconds } = useTaskStore();
          ...
          {showMenu && (
            <div className="absolute right-0 top-8 z-20 w-36 bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-zinc-100 dark:border-zinc-700 py-1 text-xs">
              <button
                ...
                className="w-full text-left px-3 py-2 hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-200 flex items-center gap-2"
              >
        ...
        {task.status === 'pending' && (
          <>
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-[#0F6E56] hover:bg-[#0c5945] text-white text-xs font-semibold flex items-center gap-1.5 shadow-sm active:scale-95 transition-all"
            >
